...
コレクションで繁用するアクションを手短に実行するために、"higher order messages"をサポートしました。`average`、`avg`、`contains`、`each`、`every`、`filter`、`first`、`flatMap`、`map`、`partition`、`reject`、`sortBy`、`sortByDesc`、`sum`コレクションメソッドでhigher order messageが使用できます。
