...
同時に、美しいキューダッシュボードと設定システムを提供する、RedisベースのLaravelキューのための[Laravel Horizon](https://horizon.laravel.com)も新たにリリースしました。
...
Horizonの詳細は、[完全なHorizonドキュメント](horizon)をご覧ください。
...
サービスプロバイダとファサードをディスカバリーで使用するための変更の詳細は、[パッケージ開発](packages)のドキュメントで確認してください。
...
もちろん、これは一番基本的なAPIリソースです。Laravelは、リソースとリソースコレクションを構築する時に役立つ、バラエティ豊かなメソッドを提供しています。詳細は、APIリソースの[完全なドキュメント](eloquent-resources)をご覧ください。
